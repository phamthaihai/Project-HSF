...
public class LessonServiceImpl implements LessonService {
    ...
    public LessonViewerDTO getLessonViewer(User loginUser, Integer courseId, Integer lessonId) {
        ...
        List<Chapter> chapters = chapterRepository
                .findByCourse_CourseIdOrderByChapterOrderAsc(courseId);

        List<Lesson> flatLessons = new ArrayList<>();
        for (Chapter chapter : chapters) {
            flatLessons.addAll(lessonRepository.findByChapter_ChapterIdOrderByOrderIndexAsc(chapter.getChapterId()));
        }
    ...
    private List<ChapterItemDTO> mapChapters(List<Chapter> chapters, List<Lesson> flatLessons, Lesson currentLesson) {
        ...
        for (Chapter chapter : chapters) {
            ...
            chapterDTO.setSortOrder(chapter.getChapterOrder());
            ...
            for (Lesson lesson : flatLessons) {
                if (lesson.getChapter() != null
                    ...
                    lessonDTO.setSortOrder(lesson.getOrderIndex());
    ...
    private LessonContentDTO toLessonContentDTO(Lesson lesson) {
        ...
        if (dto.isVideoType() && lesson.getContentVideo() != null) {
            dto.setVideoUrl(lesson.getContentVideo());
        } else if (dto.isPdfType() && lesson.getContentVideo() != null) {
            dto.setPdfUrl(lesson.getContentVideo());
        } else if (dto.isRichTextType() && lesson.getContentVideo() != null) {
            dto.setContent(lesson.getContentVideo());
        }
